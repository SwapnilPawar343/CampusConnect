...
const StudentChat = () => {
  ...
  const loadChat = async () => {
    try {
      const response = await fetch(`${backendUrl}/api/chats/student`, {
        headers: {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch(`${backendUrl}/api/chats/student/send`, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${getToken()}`,
        },
    ...
    <div className="min-h-screen bg-linear-to-br from-purple-950 via-purple-900 to-indigo-950 p-0 md:p-6">
      <div className="mx-auto flex h-screen max-w-6xl overflow-hidden bg-slate-900/70 md:h-[88vh] md:rounded-3xl md:border md:border-pink-500/30 md:shadow-2xl md:backdrop-blur-xl">
        <aside className="hidden w-80 border-r border-pink-500/20 bg-slate-900/60 p-5 text-purple-100 lg:block">
          <p className="mb-2 text-xs font-semibold uppercase tracking-[0.2em] text-pink-300">Mentor Space</p>
          <h2 className="text-2xl font-bold text-white">Student Chat</h2>
          <div className="mt-6 rounded-2xl border border-pink-500/30 bg-pink-500/10 p-4">
            <p className="text-sm text-purple-200">Connected Mentor</p>
            <p className="mt-1 text-lg font-semibold text-white">{mentor?.name || "Not connected"}</p>
            <p className="mt-1 text-xs text-purple-300">{mentor ? "Live conversation enabled" : "Request and accept mentorship first"}</p>
          </div>
          <p className="mt-6 text-sm text-purple-300">Messages sent by you are shown on the right. Mentor replies appear on the left.</p>
        </aside>

        <section className="flex w-full flex-col">
          <div className="border-b border-pink-500/20 bg-linear-to-r from-pink-600/80 to-purple-700/80 px-4 py-3 text-white md:px-5">
            <p className="font-bold text-lg">{mentor?.name || "Mentor Chat"}</p>
            <p className="text-xs text-pink-100">{mentor ? "Online" : "No mentor connected yet"}</p>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-center text-purple-300">Connect with a mentor first from recommendation requests.</p>
          ) : messages.length === 0 ? (
            <p className="text-center text-purple-300">Start your conversation.</p>
          ) : (
            messages.map((msg) => {
              const mine = String(msg.sender) === String(student._id);
              return (
